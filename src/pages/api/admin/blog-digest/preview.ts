...
import { isNull, gte, lte, desc, eq, and } from 'drizzle-orm';
...
}>, startDate: Date, endDate: Date, introduction?: string) {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { startDate, endDate, introduction } = req.body;
    ...
    // First, let's see all published posts to debug
    ...
    console.log('All published posts:', allPublishedPosts.length);
    allPublishedPosts.forEach(post => {
      const postDate = new Date(post.publishedAt);
      console.log(`- ${post.title} (${postDate.toISOString()}) - Raw value: ${post.publishedAt}`);
    });

    // Get blog posts within the date range
    console.log('Executing query with conditions:');
    console.log('- Status: published');
    console.log('- Start date:', start.toISOString());
    console.log('- End date:', end.toISOString());
    
    // First, let's test with just the status filter to see if that works
    const statusFilteredPosts = await db
      .select()
      .from(blogPosts)
      .where(
        and(
    ...
    console.log('Posts with status filter only:', statusFilteredPosts.length);

    // Now try with date range
    const posts = await db
      ...
      .where(
        and(
          isNull(blogPosts.deletedAt),
          eq(blogPosts.status, 'published'),
          gte(blogPosts.publishedAt, start),
          lte(blogPosts.publishedAt, end)
        )
      )
      .orderBy(desc(blogPosts.publishedAt));

    // Let's also try a manual filter to see if the issue is with Drizzle
    const manuallyFilteredPosts = allPublishedPosts.filter(post => {
      const postDate = new Date(post.publishedAt);
      return postDate >= start && postDate <= end;
    });

    console.log('Drizzle query found:', posts.length, 'posts');
    console.log('Manual filter found:', manuallyFilteredPosts.length, 'posts');
    console.log('Manual filtered posts:');
    manuallyFilteredPosts.forEach(post => {
      const postDate = new Date(post.publishedAt);
      console.log(`- ${post.title} (${postDate.toISOString()})`);
    });

    console.log('Found posts:', posts.length);
    posts.forEach(post => {
      const postDate = new Date(post.publishedAt);
      console.log(`- ${post.title} (${postDate.toISOString()}) - Within range: ${postDate >= start && postDate <= end}`);
    });

    // Let's also check what the actual database values look like
    ...
    console.log('- Start date (timestamp):', start.getTime());
    ...
    return res.status(200).json({ 
      ...
      emailHtml: generateEmailHtml(posts, start, end, introduction)
    });

  } catch (error) {
    console.error('Blog digest preview API error:', error);
